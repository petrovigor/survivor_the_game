...
#if UNICODE
  const std::wstring tclassname(classname.begin(), classname.end());
#else
  const std::string tclassname(classname.begin(), classname.end());
#endif

	const WNDCLASS wc = { CS_OWNDC, WindProc, NULL, NULL, 0, LoadIcon(NULL, IDI_APPLICATION), LoadCursor(NULL, IDC_ARROW), NULL, NULL, tclassname.c_str() };
	RegisterClass(&wc);
	...
	HWND hWnd = CreateWindow( tclassname.c_str(), NULL, WS_OVERLAPPEDWINDOW & ~WS_THICKFRAME & ~WS_MAXIMIZEBOX, left, top, width, height, NULL, NULL, 0, (void*)this );
  if( !hWnd ) {
    return false;
  }
